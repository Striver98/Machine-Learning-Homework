...
# 使用集成模型执行I-FGSM攻击
ensemble_ifgsm_examples, ensemble_ifgsm_acc, ensemble_ifgsm_loss = gen_adv_examples(ensemble_model, adv_loader, ifgsm, loss_fn)
print(f'ensemble_ifgsm_acc = {ensemble_ifgsm_acc:.5f}, ensemble_ifgsm_loss = {ensemble_ifgsm_loss:.5f}')

# 创建集成I-FGSM对抗样本目录并保存图像
create_dir(root, 'ensemble_ifgsm', ensemble_ifgsm_examples, adv_names)
...
for i, cls_name in enumerate(classes):
    ...
    plt.title(f'Ensemble I-FGSM\nEnsemble: {classes[predict]}: {prob:.2%}')
    plt.axis('off')
